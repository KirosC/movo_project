// This is a modified file from movo_third_party for object segmentation
// Filters for x and y direction in voxel_grid are added
// Origin location: /kinova-movo/movo_common/movo_third_party/simple_grasping/src
...
#include <simple_grasping/object_support_segmentation.h>
  ...
  double leaf_size,llimit_x, ulimit_x,llimit_y, ulimit_y, llimit_z, ulimit_z;
  std::string field_x, field_y, field_z;
  ...
  ros::NodeHandle& nh)
{
  ...
  nh.param<double>("voxel_limit_min", llimit_z, 0.0);
  nh.param<double>("voxel_limit_max", ulimit_z, 1.8);
  nh.param<std::string>("voxel_field_name", field_z, "z");
  voxel_grid_.setLeafSize(leaf_size, leaf_size, leaf_size);
  voxel_grid_.setFilterFieldName(field_z);
  voxel_grid_.setFilterLimits(llimit_z, ulimit_z);

  nh.param<double>("voxel_limit_min", llimit_y, -0.7);
  nh.param<double>("voxel_limit_max", ulimit_y, 0.7);
  nh.param<std::string>("voxel_field_name", field_y, "y");
 /* voxel_grid_.setLeafSize(leaf_size, leaf_size, leaf_size);
  voxel_grid_.setFilterFieldName(field_y);
  voxel_grid_.setFilterLimits(llimit_y, ulimit_y);
*/
  nh.param<double>("voxel_limit_min", llimit_x, 1.0);
  nh.param<double>("voxel_limit_max", ulimit_x, 2.6);
  nh.param<std::string>("voxel_field_name", field_x, "x");
  /*voxel_grid_.setLeafSize(leaf_size, leaf_size, leaf_size);
  voxel_grid_.setFilterFieldName(field_x);
  voxel_grid_.setFilterLimits(llimit_x, ulimit_x);
*/
  // segment objects
  ...
  bool output_clouds)
{
  ...
  voxel_grid_.filter(*cloud_filtered_z);

  voxel_grid_.setFilterFieldName(field_y);
  voxel_grid_.setFilterLimits(llimit_y, ulimit_y);

  voxel_grid_.setInputCloud(cloud_filtered_z);
  voxel_grid_.filter(*cloud_filtered_y);

  voxel_grid_.setFilterFieldName(field_x);
  voxel_grid_.setFilterLimits(llimit_x, ulimit_x);

  voxel_grid_.setInputCloud(cloud_filtered_y);
